...
#undef UPDATE_KEY
		...
		{
			// Sanity-check the wrapper's status
			assert( m_eStatus == E_INITIALIZED && "InputManager::GetCursorPosition - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return Point{ 0, 0 };
		...
		{
			// Sanity-check the wrapper's status
			assert( m_eStatus == E_INITIALIZED && "InputManager::GetCursorMovement - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return Vector{ 0, 0 };
