...
namespace SGD
{
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		{
			// Sanity-check the wrapper's status
			assert( m_eStatus == E_INITIALIZED && "GraphicsManager::SetPixelatedMode - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			assert( m_eStatus == E_INITIALIZED && "GraphicsManager::ShowCursor - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
		...
		bool GraphicsManager::DrawTextureSection( HTexture handle, Point position, Rectangle section, float rotation, Vector rotationOffset, Color color, Size scale )
		{
			...
			assert( section.IsEmpty() == false && "GraphicsManager::DrawTextureSection - section rectangle is empty" );
